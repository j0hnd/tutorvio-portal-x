...
class UpdateMessageTemplateRequest extends FormRequest
{
    ...
    /**
     * Get validation rules for updating a message template.
     *
    ...
    public function rules(): array
    {
        return [
            ...
            'role_visibility' => ['sometimes', 'array', 'min:1'],
    ...
    public function messageTemplatePayload(): array
    {
        ...
        if ($this->has('teacher_public_id')) {
            $teacherPublicId = $this->validated('teacher_public_id');
            $validated['teacher_id'] = $teacherPublicId === null
                ? null
                : User::query()->where('public_id', $teacherPublicId)->valueOrFail('id');
        }
